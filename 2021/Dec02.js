 ...
 * @return {boolean}
 */
const correctRepeat = (chars,target, min, max) => {
    if (chars.length < min || chars.length < max) return false;
    let repeat = 0;
    for (let index = 0; index < chars.length; index++) {
        if (chars[index] === target) repeat++;
        if (repeat > max) return false;
    }
    return repeat >= min && repeat <= max;
}

const containOnce = (chars,target, first, second) => {
    if (chars[first] !== chars[second] && (chars[first] === target ||chars[second] === target)) return true;
    return false;
}
 ...
 * @return {number}
 */
const correctPasswords = (passwords) => {
    let correct = 0;
    for (let index = 0; index < passwords.length; index++) {
        if (correctRepeat(passwords[index].password,passwords[index].target, passwords[index].min, passwords[index].max)) correct++;
    }
    return correct;
}
 ...
 * @return {number}
 */
const correctPasswords2 = (passwords) => {
    let correct = 0;
    for (let index = 0; index < passwords.length; index++) {
        if (containOnce(passwords[index].password,passwords[index].target, passwords[index].min, passwords[index].max)) correct++;
    }
    return correct;
}
...
const main = ()  => {
    let input = "15-19 k: kkkkkkkkkkkkzkkkkkkk\n" +
    ...
    console.log(correctPasswords2(convert2(input)));
}
main();